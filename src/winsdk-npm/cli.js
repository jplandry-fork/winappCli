...
async function main() {
  ...
  try {
    ...
    // Route everything else to winsdk-cli
    await callWinsdkCli(args, { verbose: true, exitOnError: true });
...
async function showCombinedHelp() {
  ...
  // Try to get help from winsdk-cli first
  try {
    const winsdkCliPath = getWinsdkCliPath();
    await new Promise((resolve, reject) => {
      const child = spawn(winsdkCliPath, ['--help'], {
        stdio: 'inherit',
        shell: false
      });
...
async function showVersion() {
  ...
  console.log(`${packageJson.description || 'Windows SDK utilities'}`);
  console.log('');
  ...
  try {
    ...
    if (!fs.existsSync(winsdkCliPath)) {
    ...
    await new Promise((resolve, reject) => {
      const child = spawn(winsdkCliPath, ['--version'], {
        stdio: 'inherit',
        shell: false
      });
